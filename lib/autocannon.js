...
const writeResult = async (handler, result, getResourceUsage) => {
  try {
  ...
  result.memory = usage.memory;
  result.cpu = usage.cpu;
...
module.exports.fire = async (opts, handler, getResourceUsage, save) => {
  const result = await run(opts);
  return save ? writeResult(handler, result, getResourceUsage) : null;
};
